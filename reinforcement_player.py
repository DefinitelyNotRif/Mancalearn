...
class ReinforcementPlayer:
    ...
    def __init__(self, player_no, **kwargs):
        ...
        state_size = 15  # 14 for each plot/bank in the board, and one more to denote the player who will move next.
    ...
    def move(self, board):
        ...
        if use_random < self.epsilon:
            decision = self.player_range[random.randint(0, 5)]
            while board[decision] == 0:
                decision = self.player_range[random.randint(0, 5)]
            reward = self._reward(board)[1][decision % 7]
        else:
            decision = greedy_move
            reward = greedy_reward
        next_state = self.move_to_state(gameplay.move(board, self.player_no, decision))
        # reward = self._reward(board)[decision % 7]
        done = gameplay.game_over(next_state[:-1])
        self.memory_buffer.append((np.append(board, self.player_no),
                                   decision,
                                   reward,
                                   next_state,
                                   done))  # State, action, reward, next state, done
        ...
        self.time += 1
        return next_state[:-1], decision, next_state[-1], done

    @staticmethod
    def move_to_state(move_tup):
        """
        Converts move_tup, a tuple of the form (board, next_player) to a state array of the
        form [b0, b1, ..., b13, next_player].
        """
        return np.append(move_tup[0], move_tup[1])
    ...
    def _compute_loss(self, experiences):
        ...
        action_idxs = actions % 7
        interim_stack = tf.stack([tf.range(q_values.shape[0]),
                                                    tf.cast(action_idxs, tf.int32)], axis=1)
    ...
    def _reward(self, board):
        ...
        """
        my_bank = self.player_range.stop
        row_before = board[self.player_range]
        hit_bank = [board[i] == my_bank - i for i in self.player_range]  # Whether
        # a move from this slot reaches the bank exactly
        possible_states = np.apply_along_axis(
            lambda i: self.move_to_state(gameplay.move(board, self.player_no, i[0])), 1,
            np.array(self.player_range).reshape(-1, 1))  # Each row is a state
        bank_gains = possible_states[:, my_bank] - board[my_bank]  # How much was added to the bank
        ...
        possible_rewards[(possible_rewards == -1) & hit_bank] = 2  # Set reward for hitting the bank
        possible_rewards[possible_rewards == -1] = bank_gains[possible_rewards == -1]
        return possible_states, possible_rewards
    ...
    def _greedy_action(self, board):
        ...
        decision = self.player_range[int(tf.argmax(q_options))]
        return decision, possible_rewards[decision % 7]
